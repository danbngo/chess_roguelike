...
// molten overstay. Phases: grace 0-95 (first danger tick at 95), dread ramp 95-190 (max dread + first
// lava at 190), lava ramp 190-285 (molten peak at 285). The grace holds 5 steps, the ramp 5 (the ramp
// is fixed by the audio's HURRY gears; the GRACE is free). The step was 24, SPED UP to 19 (2026-08-08,
// ~1.25x faster overall). The grace was briefly CUT to 3 steps (2026-08-09) but ROLLED BACK to 5
// (2026-08-11) — with the faster step, a 3-step grace made dread bite too soon. Everything downstream
// (MAX_TURNS_SCARY, the lava overstay, the music gears via dreadFraction) derives from these.
const DREAD_STEP_TURNS = 19;
const DREAD_GRACE_STEPS = 5; // the quiet opening — nothing stirs (5 * 19 = 95 turns)
...
const DREAD_GRACE_TURNS = DREAD_STEP_TURNS * DREAD_GRACE_STEPS; // 95 — the first danger tick lands here
const MAX_TURNS_SCARY = DREAD_STEP_TURNS * DREAD_TOTAL_STEPS; // 190 — dread maxes, and the first lava wells up
// PAST max dread, a floor the king WILL NOT LEAVE turns lethal: five more OVERSTAY steps (turns
// 190-285) over which LAVA wells up and closes in (tickLavaEncroachment) and the swarm runs unbounded.
// It peaks at turn 285 — by then the molten floor and the horde together kill any build. See
...
const MAX_TURNS_LAVA = MAX_TURNS_SCARY + DREAD_STEP_TURNS * DREAD_OVERSTAY_STEPS; // 285 — the molten floor peaks here
...
const CLASSES = {
  warrior: {
    ...
    startPerk: { id: 'w_discipline', name: 'Discipline', short: 'Hold your ground to let a foe in sight come to you', desc: 'Hold your ground — tap your king (or press Space) to skip a turn and let foes in sight come to you. Refused with an empty screen, so it never quietly burns your turns. A plainer boon than the others: the Warrior is strong enough already.', grants: { discipline: true } },
    perks: [
      // ⛨ Sentinel — the immovable bastion: brace for a blow, guard the next, punish the rest.
      { id: 'w_waiting', chain: 'Sentinel', tier: 1, name: 'Brace', short: 'Hold your ground to raise a guard that blocks the next hit', desc: 'Hold your ground (Discipline — tap your king, or press Space) and you raise your GUARD (a shield shows over your token). The next blow that would land, from any foe, is turned aside and the guard drops. Bank a block, then take the fight to them. (Parry, tier 2, extends this to EVERY turn you end without striking.)', grants: { waiting: true } },
      { id: 'w_bulwark', chain: 'Sentinel', tier: 2, requires: 'w_waiting', name: 'Parry', short: 'Raise the guard on ANY turn you end without attacking', desc: 'Brace made you raise your guard by deliberately waiting; now you raise it whenever you END a turn without striking — a plain step, a wade, a reposition all bank the block. The next blow that would land is turned aside and the guard drops, whatever you did in between. Bank it, then take the fight to them.', grants: { firstHitEachTurn: true } },
  ...
  ranger: {
    ...
    perks: [
      ...
      { id: 'r_eagle', chain: 'Oracle', tier: 1, name: 'Premonition', short: 'See through ANY cover; shoot only through haze — grass & steam (one-way)', desc: 'Your sight passes straight through ANY cover within your radius — walls, boulders, trees, tall grass, erupting steam — nothing hides a foe from you. But you can only SHOOT through what would not stop you WALKING (grass, steam): an arrow still slaps into solid cover, so a foe glimpsed through a wall is spotted, not a target. Ground you can SEE but not shoot to is dimmed. ONE-WAY: those foes can’t strike you back, but they DO wake and start closing in', grants: { trueSight: true } },
