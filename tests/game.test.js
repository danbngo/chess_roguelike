...
const {
  ...
  buildTutorialFloor, tickTutorial, tutJuncture, leaveTutorial,
...
test('a danger event always LANDS — a fizzled pick is retried, never spent', () => {
  ...
  const snap = (st) => JSON.stringify({
    ...
    pieces: st.enemies.map((e) => `${e.kind}${e.turret ? 'T' : ''}${e.summonCircle ? 'C' : ''}${e.mini ? 'M' : ''}@${e.x},${e.y}`).sort().join('|'),
...
test('dread conversions: foes in view become mini-bosses or summoning runes — and fizzle on an empty board', () => {
  const crowd = () => {
    const s = createInitialState('warrior');
    s.terrain = {}; s.allies = []; s.key = null; s.upstair = null;
    s.player.x = 10; s.player.y = 10; s.player.vision = 40; // see the whole cluster
    s.enemies = [];
    for (let i = 0; i < 4; i += 1) s.enemies.push(createEnemy('rook', 13 + i, 10)); // four rooks in view, not adjacent
    return s;
  };
  // ANOINT — ordinary foes swell into hunting mini-bosses where they stand.
  {
    const s = crowd();
    assert.ok(enemiesToMinis(s), 'anoint reports');
    const minis = s.enemies.filter((e) => e.mini && e.boss);
    assert.ok(minis.length >= 1 && minis.length <= 4, `some foes anointed (${minis.length})`);
    assert.ok(minis.every((m) => m.awake), 'and they hunt at once');
  }
  // RUNECIRCLES — foes sink into summoning runes, each with a same-class minion already beside it.
  {
    const s = crowd();
    const before = s.enemies.length;
    assert.ok(enemiesToCircles(s), 'runecircles reports');
    assert.ok(s.enemies.some((e) => e.summonCircle), 'a foe became a summoning circle');
    assert.ok(s.enemies.length > before, 'and a minion rose beside it (the board grew)');
    assert.ok(s.enemies.some((e) => e.summoned && e.kind === 'rook'), 'the minion is of the SAME class');
  }
  // Neither fires on a bare board — it returns '' so the danger roll re-rolls onto something that lands.
  {
    const s = createInitialState('warrior');
    s.terrain = {}; s.enemies = []; s.allies = [];
    assert.equal(enemiesToMinis(s), '', 'anoint fizzles with no foes');
    assert.equal(enemiesToCircles(s), '', 'runecircles fizzles with no foes');
  }
});
...
test('THE TRAINING GROUNDS — a winding, spread-out floor: shielded signs, then LIVE threats', () => {
  ...
  assert.equal(en(s, 'tut-foe').kind, 'ferz', 'the foe is a ferz — a clean one-step diagonal mover (renders as a bishop sprite)');
  ...
  { let p = buildTutorialFloor('warrior', 'hard'); p.player.x = 13; p.player.y = 5; const m = en(p, 'tut-foe'); m.asleep = false; m.awake = true; const hp = p.player.hp; let hit = false; for (let t = 0; t < 4 && !p.gameOver; t += 1) { p = live(p); if (p.player.hp < hp) { hit = true; break; } } assert.ok(hit || p.gameOver, 'the ferz strikes the diagonally-adjacent king'); }
...
test('THE TRAINING GROUNDS — the guided-spotlight JUNCTURE fires at each tricky moment, and only there', () => {
  ...
  const abil = tutJuncture(put(s, 19, 4));
  assert.ok(abil && abil.kind === 'ability', 'on the launch tile with the barrier just east → ABILITY juncture');
  // the TURRET room is deliberately NOT gated — no single tile is the one right answer there.
  assert.equal(tutJuncture(put(buildTutorialFloor('warrior', 'hard'), 13, 12)), null, 'the turret room stays free');
  s.enemies = s.enemies.filter((e) => e.id !== 'tut-foe');
  const circ = tutJuncture(put(s, 3, 12));
  assert.ok(circ && circ.kind === 'circle' && circ.x === 2 && circ.y === 12, 'adjacent to the circle → shatter it');
  s.enemies = s.enemies.filter((e) => e.id !== 'tut-circle');
  const keyj = tutJuncture(put(s, 12, 21));
  assert.ok(keyj && keyj.kind === 'key' && keyj.x === 12 && keyj.y === 20, 'adjacent to the uncollected key → grab it');
  s.key.collected = true;
  assert.ok(!(tutJuncture(put(s, 12, 21)) || {}).kind || (tutJuncture(put(s, 12, 21)).kind !== 'key'), 'once collected, the key is no longer a juncture');
  const stairj = tutJuncture(put(s, 16, 21));
  assert.ok(stairj && stairj.kind === 'stair' && stairj.x === 16 && stairj.y === 20, 'key in hand, adjacent to the stair → descend');
  // Off the training floor the detector is inert — the real game is never gated.
  assert.equal(tutJuncture(createInitialState('warrior', 'hard')), null, 'no junctures in the real dungeon');
});
